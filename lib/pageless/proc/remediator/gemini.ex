defmodule Pageless.Proc.Remediator.Gemini do
  @moduledoc "Gemini prompt and tool schema helpers for remediator agents."

  alias Pageless.AlertEnvelope

  @valid_classes ~w(read write_dev write_prod_low write_prod_high)a

  @doc "Builds the Gemini generation options for a remediation proposal request."
  @spec opts(AlertEnvelope.t(), [map()]) :: keyword()
  def opts(envelope, findings) do
    [
      model: :pro,
      temperature: 0.0,
  ...
  @doc "Returns the remediator system instruction used for Gemini proposal generation."
  ...
  @doc "Returns the Gemini function declaration for remediation proposals."
  @spec propose_action_tool() :: map()
  def propose_action_tool do
    %{
  ...
  @doc "Summarizes an alert envelope for Gemini prompts and spawn events."
  ...
  @doc "Summarizes investigator findings for remediator spawn events."
